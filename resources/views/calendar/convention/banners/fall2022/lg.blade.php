<style>
@import url('https://fonts.googleapis.com/css?family=Federo&display=swap');

@import url('https://fonts.googleapis.com/css?family=Advent+Pro&display=swap');

  /* Convention Banner */
  .banner {
    font-family: 'Raleway', sans-serif;
    /* margin-bottom: 0px; */
    background: url(/img/fall2022.jpg) center bottom no-repeat ;
    box-shadow: inset 0px 0px 300px rgba(0, 0, 0, 0.50);
    background-size: cover;
    color: white;
    text-shadow: black 0.1em 0.1em 0.1em;
    position: relative; 
  }
  
  .quote {
      line-height: 300%;
      text-align: center;
  }

  .quote-font{
    font-size: 8rem; 
    color: #fde947;
    text-shadow: rgb(230, 107, 107) 0.1em 0.1em 1.4em;
    opacity: 0.4;
    transform: scaleY(1.4);
    font-family: 'Federo', sans-serif;
  }

  .logo{
    font-size: 1.2rem;
    font-weight: bold;
    color: #d2d8d3;
    margin: 0;
    padding: 0;
  }
  
  .date{
    font-size: .8rem; 
  }
  
  .logo .con{
    color:#b9bab2; 
    font-weight: bolder;
    font-weight:500
  }

  .row {
    min-height: 400px;
  }

.banner-footer {
  position: absolute;
  bottom: 0;
  width: 100%;
  height: 3rem;            /* Footer height */
}

</style>
<div class="col-10 offset-1">
  <div class='row'>
      <div class='col banner-content-wrap'>
          <div class="quote-font text-right" >
              <div class="quote">Saviors</div>
          </div>
          <div class=" banner-footer">
              <div class="row">
                  <div class="col logo text-left">
                      Intrigue<span class='con'>Con</span> X  | <span class="date">October 14th to 16th</span>
                  </div>
                  <div class="col text-right">
                      <a href="/calendar/convention" class="btn btn-outline-light btn-sm">Find Out More</a>
                  </div>
              </div>
          </div>
      </div> 
  </div>
</div>

