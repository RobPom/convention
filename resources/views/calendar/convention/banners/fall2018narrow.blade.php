<style>

  /* Convention Banner */
  .banner {
    font-family: 'Raleway', sans-serif;
    margin-bottom: 0px;
    background: url(/img/banner-background.jpg) left center no-repeat ;
    /*background: url(/img/lil_nap.png) right bottom no-repeat, url(/img/paper_texture_02.jpg) no-repeat ;*/
    box-shadow: inset 0px 0px 300px rgba(0, 0, 0, 0.50);
    background-size: cover;
    color: white;
    text-shadow: black 0.1em 0.1em 0.1em;
  }
  
  .banner.fall  {
   
     min-height: 200px; 
  
  }

  .fall .quote-font{
    font-family: 'Amatic SC', cursive;
    font-size: 1.6rem;
    color: #f4e1c4;
  }
  
  .fall .logo{
    font-size: 1.3rem;
    font-weight: 300;
  }
  
  .fall .date{
    font-size: .8rem;
  
  }
  
  .fall .logo .con{
    color:yellow; 
    font-weight:500
  }
  
  .aniron {
    font-family: Aniron;
    color: #88c655;
  }
  .aniron_sm {
    font-size: .3em;
  }
  
  .aniron_lrg {
    font-size: .7em;
  }
</style>
<div class="banner fall p-4">
    <div class='row'>   
        <div class="logo text-center col-12">
            Intrigue<span class='con'>Con</span> 5
        </div>
        <div class="date text-center col-12">
            Oct 12 - 14, 2018
        </div>

        <div class="quote-font text-center" >
        ... for when you gaze long into the abyss. The abyss gazes also into you.  

        </div>  
        
        
    </div>
    <br>
    <div class='row text-center'>
      <div class="col-12 text-center">
          <a href="/calendar/convention" class="btn btn-info btn-sm mt-2" style="background-color: #2c6459 ; border-color: #642c37">Find out More</a> 
          <div><strong>Sold Out!</strong></div>
          <!-- <a href="/calendar/convention/1/register" class="btn btn-info btn-sm mt-2" >Register Now!</a> -->
      </div>
        
    </div>
</div>